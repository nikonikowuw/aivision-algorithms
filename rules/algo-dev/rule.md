...
在 `algorithms/` 目录下需要确保所有算法包都遵循统一的 ABI 接口规范、提供完整的元数据描述，并且包含必要的自检机制。
...
3. **构建验证**: 维护并使用 `CMakeLists.txt`编译生成 `.so`。
4. **测试自检**: 运行单元测试 `make test`，确保新生成的动态库输出了正确的符号并且可以成功完成本地推理。
