...
def parse_args():
    parser = argparse.ArgumentParser(
        description="SCRFD Model Converter for Apple Silicon (CoreML), Rockchip (RKNN), and Huawei Ascend (OM)"
    )
    parser.add_argument(
        "--input", "-i",
        type=str,
        default="weights/scrfd_500m.onnx",
        help="Path to input ONNX model. Default: weights/scrfd_500m.onnx"
    )
    parser.add_argument(
        "--target", "-t",
        type=str,
...
def convert_to_coreml(onnx_path, output_path, imgsz):
    ...
    print(f"🔄 Converting SCRFD ONNX model {onnx_path} to CoreML (.mlpackage)...")
    try:
...
def main():
    ...
    if not input_path.exists():
        ...
        print("💡 Hint: Please run the export script to generate the ONNX file first, or download it:", file=sys.stderr)
